...
def username():
    username = tk.Toplevel()
    username.title("WHITEWOLF-TOOLS")
    username.geometry("1200x560")
    username.minsize(1200, 560)
    username.iconbitmap("icon-tool.ico")
    username.configure(bg="black")
    username.grid_columnconfigure(0, weight=1)
    username.grid_columnconfigure(1, weight=1)
    username.grid_rowconfigure(0, weight=1)

    hautdroite_frame = Frame(username, bg="black")
    hautdroite_frame.grid(row=0, column=1, sticky="ne", padx=(0, 150))

    hautdroite2_frame = Frame(username, bg="black")
    hautdroite2_frame.grid(row=0, column=1, sticky="e", padx=(0, 40))

    bg_label = tk.Label(username, bg="black")
    ...
    USERNAME = Entry(hautdroite2_frame, font=("Orbitron", 14, "bold"), fg="white", bg="#1a1a1a", insertbackground="#00ffff", relief="flat", bd=0, width=20, justify="center")
    USERNAME.grid(row=0, column=0, columnspan=2, padx=10, pady=10, ipady=8)
    USERNAME.insert(0, "Entrez un pseudo")

    def confirm():
        username = USERNAME.get()
        fulluser_name(username, result_label)
